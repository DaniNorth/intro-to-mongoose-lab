...
const viewCustomers = async () => {
    ...
    customers.forEach(cust => {
        console.log(id: ${cust._id} -- Name: ${cust.name}, Age: ${cust.age}, location: ${cust.location});
    });
};
...
const updateCustomer = async () => {
    ...
    // perform the update
    const location = prompt('What is the customers new location?');
...
const runProgram = async () => {
    ...
    while (command !== 5) {
        ...
        Number of action to run: );
            ...
            const command = parseInt(command); // convert string input into integer
